...
public class CustTabAttributes {
    public static void populateAttributeList(Activity activity, String screenName, int screenID, RecyclerView recyclerView, MetrixTabScreenManager.TabChildInfo tci, String maxRows, String searchCriteria) {
        try {
            ...
            if (recyclerView != null) {
                ...
                recyclerView.setVisibility(listCount > 0 ? View.VISIBLE : View.GONE);
            } else {
    ...
    private static List<HashMap<String, String>> getAttributeListData(String screenName, int screenID, String maxRows, String searchCriteria) {
            ...
            String query = MetrixListScreenManager.generateListQuery("attribute",
                    "attribute.table_name = 'PLACE' and attribute.foreign_key_char1 = '" + placeId + "' order by attribute.metrix_row_id desc", searchCriteria, screenID);
            ...
            try {
                ...
                while (cursor.isAfterLast() == false) {
                    ...
                    String attrValueDesc = MetrixDatabaseManager.getFieldStringValue("attribute_value_lkup_view", "description", String.format("table_name = 'PLACE' and attr_name = '%1$s' and code_value = '%2$s'", attrName, attrValue));
