...
public class DebriefCustomerReview extends DebriefActivity implements View.OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(shouldRunTabletSpecificUIMode)
            setContentView(R.layout.tb_land_debrief_customer_review);
        else
            setContentView(R.layout.debrief_customer_review);
    ...
    private void updateControlState() {
        ...
        boolean attachmentAttachmentIDExists = false;

        try {
            String sqlCheckattachmentId = "select attachment_id from attachment LEFT OUTER JOIN task on task.cust_signature_id = attachment.attachment_id where task.task_id = '"+taskId+"' " ;
            ArrayList<Hashtable<String, String>> results = MetrixDatabaseManager.getFieldStringValuesList(sqlCheckattachmentId);
            if (results != null && !results.isEmpty()) {
                String attachmentId = results.get(0).get("attachment_id");
                attachmentAttachmentIDExists = !MetrixStringHelper.isNullOrEmpty(attachmentId);
            }

            View signatureCtrl = MetrixControlAssistant.getControl(mFormDef, mLayout, "task", "cust_signature_id");
            if (signatureCtrl != null){
                String signatureId = MetrixControlAssistant.getValue(signatureCtrl);
                mSignatureSaved = !MetrixStringHelper.isNullOrEmpty(signatureId);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }

        boolean enabled = !attachmentAttachmentIDExists || !mSignatureSaved;
    ...
    private boolean preCheck(boolean advanceWorkflowOnError){
    ...
    public boolean save(boolean advanceWorkflowOnError) {
        try {
            ...
            MetrixUpdateManager.update(this, mLayout, mFormDef, transactionInfo, false, null, false, AndroidResourceHelper.getMessage("CustomerSignature"));
            return true;
        } catch (Exception e) {
    ...
    protected void defaultValues() {
        ...
        try {
            ...
            if (totalExpenses != null && totalLabor != null && totalParts != null && (totalExpenses.size() > 1 || totalLabor.size() > 1 || totalParts.size() > 1)) {
                MetrixControlAssistant.setValue(mFormDef, mLayout, "custom", "total", DebriefCustomerReview.NOT_APPLICABLE);
                MetrixControlAssistant.setValue(mFormDef, mLayout, "custom", "outstanding", DebriefCustomerReview.NOT_APPLICABLE);
            } else {
