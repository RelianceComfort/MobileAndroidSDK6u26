...
public class MetadataScheduleActivity extends ScheduleActivity implements View.OnClickListener, View.OnFocusChangeListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		try {
			...
			setContentView(R.layout.yycsmd_schedule_activity);
	...
	protected void defineForm() {
		MetrixTableDef requestContactDef = new MetrixTableDef(primaryTable.toLowerCase(), MetrixTransactionTypes.INSERT);
		this.mFormDef = new MetrixFormDef(requestContactDef);
	}
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
			case R.id.save:
				...
				MetrixSaveResult saveResult = MetrixUpdateManager.update(this, mLayout, mFormDef, transactionInfo, false, null, false, String.format("MSA-%s", codeLessScreenName));
				if (saveResult == MetrixSaveResult.ERROR)
					return;

				//we should show the same/current page
				intent = MetrixActivityHelper.createActivityIntent(this, MetadataScheduleActivity.class);
			...
			case R.id.next:
				if (anyOnStartValuesChanged()) {
					transactionInfo = MetrixTransaction.getTransaction("request", "request_id");
					saveResult = MetrixUpdateManager.update(this, mLayout, mFormDef, transactionInfo, true, null, false, String.format("MSA-%s", codeLessScreenName), true);
				...
				if (!MetrixStringHelper.isNullOrEmpty(linkedScreenId)) {
					...
					if (!MetrixStringHelper.isNullOrEmpty(screenType)) {
						if (screenType.toLowerCase().contains("list")) {
							intent = MetrixActivityHelper.createActivityIntent(this, MetadataListScheduleActivity.class);
			...
			case R.id.update:
				if (anyOnStartValuesChanged()) {
					transactionInfo = MetrixTransaction.getTransaction("request", "request_id");
					saveResult = MetrixUpdateManager.update(this, mLayout, mFormDef, transactionInfo, true, null, false, String.format("MSA-%s", codeLessScreenName));
					if (saveResult == MetrixSaveResult.ERROR || saveResult == MetrixSaveResult.ERROR_WITH_CONTINUE)
						return;
			...
			case R.id.correct_error:
				transactionInfo = MetrixTransaction.getTransaction("request", "request_id");
				saveResult = MetrixUpdateManager.update(this, mLayout, mFormDef, transactionInfo, false, SyncServiceMonitor.class, true, String.format("MSA-%s", codeLessScreenName));
				if (saveResult == MetrixSaveResult.ERROR)
				...
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int whichButton) {
						if (finalNextActivity != null) {
							...
							if (isStandardUpdateOnly) {
								//we should show the same/current page
								Intent intent = MetrixActivityHelper.createActivityIntent(currentActivity, MetadataScheduleActivity.class);
								intent.putExtra("ScreenID", codeLessScreenId);
								MetrixActivityHelper.startNewActivityAndFinish(currentActivity, intent);
							} else
								currentActivity.finish();
