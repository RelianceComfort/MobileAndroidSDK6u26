...
public class MetadataListScheduleActivity extends ScheduleActivity implements View.OnClickListener, MetrixRecyclerViewListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.yycsmd_list_schedule_activity);
				...
				if (!MetrixStringHelper.isNullOrEmpty(linkedScreenId)) {
					...
					if (!MetrixStringHelper.isNullOrEmpty(screenType)) {
						if (screenType.toLowerCase().contains("standard")) {
							Intent intent = MetrixActivityHelper.createActivityIntent(this, MetadataScheduleActivity.class);
	...
	public void onListItemClick(int position, @NonNull HashMap<String, String> listItemData, @NonNull View view) {
		...
		//Modifying is not yet supported.
		...
		final OnClickListener deleteListener = (dialog, which) -> {
			try {
				...
				MetrixUpdateManager.delete(MetadataListScheduleActivity.this, primaryTable, metrixRowIdValue, keys, String.format("MLSA-%s", codeLessScreenName), transactionInfo);
		...
		if (allowModify && allowDelete)
			//Although modifying is allowed - we are not yet there. Only delete is allowed
			...
			//Although modifying is allowed - we are not yet there.
			MetrixUIHelper.showSnackbar(mCurrentActivity, R.id.coordinator_layout, AndroidResourceHelper.getMessage("YYCSListItemModifiedNotAllowed"));
		else if (!allowModify && allowDelete)
			MetrixDialogAssistant.showConfirmDeleteDialog(AndroidResourceHelper.getMessage("YYCSListItmModDelItmTxt"), deleteListener, null, this);
		else
