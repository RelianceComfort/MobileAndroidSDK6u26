 ...
 */
public class SimpleAdapterSpannable extends BaseAdapter implements Filterable {
    private int[] mTo;
     ...
     */
    public SimpleAdapterSpannable(Context context, List<? extends Map<String, ?>> data,
                ...
                if (!bound) {
                    if (v instanceof Checkable) {
                        if (data instanceof Boolean) {
                            ...
                            throw new IllegalStateException(AndroidResourceHelper.getMessage("ShouldBeBoundToA2Args",
                                    v.getClass().getName(), (data == null ? AndroidResourceHelper.getMessage("UnknownType") : data.getClass())));
                        }
                    } else if (v instanceof TextView) {
                        // Note: keep the instanceof TextView check at the bottom of these
                        // ifs since a lot of views are TextViews (e.g. CheckBoxes).
                    	if (data instanceof Spannable)
                    	{
                    		TextView tv = (TextView)v;
                    		tv.setText((Spannable)data);
                    	}
                    	else
                    	{
                    		setViewText((TextView) v, text);
                        ...
                        throw new IllegalStateException(AndroidResourceHelper.getMessage("CantBeBoundBySimpleAdapter", v.getClass().getName()));
                    }
                }
            }
        }
