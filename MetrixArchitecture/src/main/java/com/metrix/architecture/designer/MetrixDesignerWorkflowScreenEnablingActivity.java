...
public class MetrixDesignerWorkflowScreenEnablingActivity extends MetrixDesignerActivity {
	private List<HashMap<String, String>> mOriginalData;
	private ListView mListView;
	private TextView mTitle;
	private Button mSave, mNext;
	private String mWorkflowName;
	private WorkflowScreenCheckListAdapter mWorkflowChkAdapter;
	private int mMaxStepOrder = 0;
	private MetrixDesignerResourceData mWorkflowSEResourceData;

	@Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);

        mWorkflowSEResourceData = (MetrixDesignerResourceData) MetrixPublicCache.instance.getItem("MetrixDesignerWorkflowScreenEnablingActivityResourceData");

        setContentView(mWorkflowSEResourceData.LayoutResourceID);

        mListView = (ListView) findViewById(mWorkflowSEResourceData.ListViewResourceID);
    }
    ...
    public void onStart() {
		...
		helpText = mWorkflowSEResourceData.HelpTextString;
		...
		mTitle = (TextView) findViewById(mWorkflowSEResourceData.getExtraResourceID("R.id.zzmd_workflow_screen_enabling_title"));
        String fullTitle = AndroidResourceHelper.getMessage("Screens1Args", mWorkflowName);
        mTitle.setText(fullTitle);

		mSave = (Button) findViewById(mWorkflowSEResourceData.getExtraResourceID("R.id.save"));
		mSave.setEnabled(mAllowChanges);
        mSave.setOnClickListener(this);

        mNext = (Button) findViewById(mWorkflowSEResourceData.getExtraResourceID("R.id.next"));
        mNext.setOnClickListener(this);

        TextView mScrInfo = (TextView) findViewById(mWorkflowSEResourceData.getExtraResourceID("R.id.screen_info_metrix_designer_workflow_screen_enabling"));

        AndroidResourceHelper.setResourceValues(mScrInfo, "ScnInfoMxDesWfScnEnable");
...
//		query.append(" and mm_screen.workflow_id = " + MetrixCurrentKeysHelper.getKeyValue("mm_workflow", "workflow_id"));
		...
		try {
			...
			while (cursor.isAfterLast() == false) {
				...
				String thisStepOrder = cursor.getString(3);
                int thisStepOrderNum = Integer.valueOf(thisStepOrder);
                ...
                row.put("mm_workflow_screen.step_order", thisStepOrder);
				row.put("mm_screen.force_order", thisForceOrder);

				if (thisStepOrderNum > 0) {
                ...
                origRow.put("mm_workflow_screen.step_order", thisStepOrder);
				origRow.put("mm_screen.force_order", thisForceOrder);
				...
				if (thisStepOrderNum > mMaxStepOrder) {
					mMaxStepOrder = thisStepOrderNum;
				}

				cursor.moveToNext();
			}

			mWorkflowChkAdapter = new WorkflowScreenCheckListAdapter(this, table, mWorkflowSEResourceData.ListViewItemResourceID, mWorkflowSEResourceData.ExtraResourceIDs, mAllowChanges);
	...
	public void onClick(View v) {
		...
		int viewId = v.getId();
        if (viewId == mWorkflowSEResourceData.getExtraResourceID("R.id.save")) {
            if (mAllowChanges) {
                //String message = AndroidResourceHelper.getMessage("ScreenEnablingTip");
        ...
        } else if (viewId == mWorkflowSEResourceData.getExtraResourceID("R.id.next")) {
            if (mAllowChanges) {
				processAndSaveChanges();
            }
    ...
    private void processAndSaveChanges() {
		...
		*/
            ...
            if (idMapIndex > -1) {
				...
				if (!MetrixStringHelper.valueIsEqual(origCheckboxState, chgCheckboxState)) {
					if (MetrixStringHelper.valueIsEqual(chgCheckboxState, "Y")) {
						mMaxStepOrder++;
						chgRow.put("mm_workflow_screen.step_order", String.valueOf(mMaxStepOrder));
						mWorkflowChkAdapter.mIdMap.put(chgRow, idMapIndex);
					} else {
						int negativeOne = -1;
						chgRow.put("mm_workflow_screen.step_order", String.valueOf(negativeOne));
                        mWorkflowChkAdapter.mIdMap.put(chgRow, idMapIndex);
                    }
				}

                if (MetrixStringHelper.valueIsEqual(chgCheckboxState, "Y")) {
					// add to TreeMap, which auto-sorts by step_order property value
					int currValue = Integer.valueOf(chgRow.get("mm_workflow_screen.step_order"));
		...
		for (Integer sequence : positiveOrderSet.keySet()) {
            ...
            if (idMapIndex2 > -1) {
				chgRow2.put("mm_workflow_screen.step_order", String.valueOf(counter));
		...
		for (HashMap<String, String> origRow : mOriginalData) {
			String origScreenID = origRow.get("mm_workflow_screen.screen_id");
			String origOrder = origRow.get("mm_workflow_screen.step_order");

            for (HashMap<String, String> listRow : mWorkflowChkAdapter.mListData) {
				String listScreenID = listRow.get("mm_workflow_screen.screen_id");
				if (MetrixStringHelper.valueIsEqual(origScreenID, listScreenID)) {
					String listOrder = listRow.get("mm_workflow_screen.step_order");
					if (!MetrixStringHelper.valueIsEqual(origOrder, listOrder)) {
						...
						data.dataFields.add(new DataField("step_order", listOrder));
    ...
    public static class WorkflowScreenCheckListAdapter extends DynamicListAdapter {
		static ViewHolder holder;
		private final String checkBoxText;

		public WorkflowScreenCheckListAdapter(Context context, List<HashMap<String, String>> table, int listViewItemResourceID, HashMap<String, Integer> lviElemResIDs, boolean allowChanges) {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
            ...
            holder.mStepOrder.setText(dataRow.get("mm_screen.step_order"));
            String forceOrder = dataRow.get("mm_screen.force_order");
            ...
            if (!MetrixStringHelper.isNullOrEmpty(forceOrder)) {
				holder.mBox.setEnabled(false);
			} else {
				holder.mBox.setEnabled(true);
			}
