 ...
 * 
 * This file was renamed from IntentIntegrator to MetrixBarcodeAssistant
 ...
 * @since 5.6
 */
public class MetrixBarcodeAssistant {

	public static final int REQUEST_CODE = 0x0000c0de; // Only use bottom 16
	// bits
	private static final String TAG = MetrixBarcodeAssistant.class.getSimpleName();
	 ...
	 * @since 5.6
	 */
	public MetrixBarcodeAssistant(Activity activity) {
		this.activity = activity;
		title = AndroidResourceHelper.getMessage("BarcodeScannerTitle");
		message = AndroidResourceHelper.getMessage("BarcodeScannerMessage");
		buttonYes = AndroidResourceHelper.getMessage("Yes");
		buttonNo = AndroidResourceHelper.getMessage("No");
	...
	@Deprecated
	public AlertDialog initiateScan(int id) {
		Collection<String> desiredBarcodeFormats = ALL_CODE_TYPES;
		...
		intentScan.putExtra(MetrixBarcodeAssistant.VIEW_ID, id);
	...
	/**
	 * Scans a barcode and immediately sets the value in the supplied TextView.
	 * On failure or when scanning is cancelled this method will do nothing.
	 * @param viewToSet
	 */
	public static void scanBarcode(final TextView viewToSet) {
		if (viewToSet == null) {
			return;
		}
		GmsBarcodeScanner scanner = GmsBarcodeScanning.getClient(viewToSet.getContext());
		scanner
				.startScan()
				.addOnSuccessListener(
						barcode -> {
							if (viewToSet != null) {
								viewToSet.setText(barcode.getRawValue());
							}
						})
				.addOnCanceledListener(() -> {})
				.addOnFailureListener(e -> {
					LogManager.getInstance().error(e);
					Toast.makeText(viewToSet.getContext(), e.getMessage(), Toast.LENGTH_SHORT).show();
				});
	}

	/**
	 * Checks for the availability of Google code scanner and requests
	 * an immediate installation if not available.
	 * @param context
	 */
	public static void makeGoogleCodeScannerAvailable(Context context) {
		...
		moduleInstallClient.areModulesAvailable(scanner).addOnSuccessListener(response -> {
			if (!response.areModulesAvailable()) {
				ModuleInstallRequest installRequest = ModuleInstallRequest.newBuilder().addApi(scanner).build();
		...
		downloadDialog.setPositiveButton(buttonYes, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialogInterface, int i) {
				...
				try {
					...
					LogManager.getInstance().warn(TAG + " Android Market is not installed; cannot install Barcode Scanner");
				}
			}
		});
	 ...
	 */
	public static MetrixBarcodeScanResult parseActivityResult(int requestCode, int resultCode, Intent intent) {
		if (requestCode == REQUEST_CODE) {
			if (resultCode == Activity.RESULT_OK) {
				...
				return new MetrixBarcodeScanResult(contents, formatName, rawBytes, orientation, errorCorrectionLevel);
			}
			return new MetrixBarcodeScanResult();
		}
		return null;
	}
